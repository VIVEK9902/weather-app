    ...
    public ResponseEntity<?> get7DayForecast(
        ...
        try {
            ...
            if (locationObj instanceof Map) {
                ...
                out.put("lat", loc.getOrDefault("lat", null));
                out.put("lon", loc.getOrDefault("lon", null));
                out.put("timezone", loc.getOrDefault("tz_id", resp.get("timezone")));
            } else {
                out.put("lat", lat);
                out.put("lon", lon);
            }
            ...
            for (Object o : forecastDays) {
                ...
                if (dayObj instanceof Map) {
                    ...
                    if ("imperial".equalsIgnoreCase(units)) {
                        ...
                        // wind in mph
                        dayOut.put("wind_speed", d.getOrDefault("maxwind_mph", d.get("maxwind_kph")));
                    } else {
                        // metric
                        ...
                        // wind in kph
                        dayOut.put("wind_speed", d.getOrDefault("maxwind_kph", d.get("maxwind_mph")));
                    }
                    // humidity
